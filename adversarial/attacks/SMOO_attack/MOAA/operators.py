...
def mutation(soln: Solution, pm: float, all_pixels: np.array, zero_prob: float, num_channels: int):
    ...
    rgbs_ = np.random.choice([-1, 1, 0], size=(eps_it, num_channels), p=(ones_prob, ones_prob, zero_prob)) # Updated num_channels for 1 channel image
...
def generate_offspring(parents, pc, pm, all_pixels, zero_prob, num_channels):
    children = []
    for pi in parents:
        offspring1, offspring2 = crossover(pi[0], pi[1], pc)

        mutation(offspring1, pm, all_pixels, zero_prob, num_channels)
        mutation(offspring2, pm, all_pixels, zero_prob, num_channels)
