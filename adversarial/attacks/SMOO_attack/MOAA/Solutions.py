...
class Solution:
    def __init__(self, pixels, values, x, p_size):
        ...
        self.x = x  # (H, W, C) format  --> different than original code which had square image
        self.fitnesses = []
        self.is_adversarial = None
        self.h = x.shape[0]   # Height (number of rows) # Added for voter's dataset
        self.w = x.shape[1]   # Width (number of columns) # Updated for voter's dataset
    ...
    def generate_image(self):
        ...
        for i in range(self.delta):
            ...
            row = self.pixels[i] // self.w   # Divide by WIDTH
            col = self.pixels[i] % self.w    # Modulo by WIDTH
