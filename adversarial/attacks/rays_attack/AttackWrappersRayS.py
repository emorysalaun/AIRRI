...
def RaySAttack(device, model, epsMax, queryLimit, cleanLoader):
    ...
    for i in range(0, xClean.shape[0]):
        ...
        # yCurrent[0] =
        xAdvCurrent, stop_queries, dist, isLessDist = rayS.attack_hard_label(
            xClean[i].to(device),
            yClean[i].to(device),
            target=None,
        ...
        if (
            dist > epsMax
        ):  # -----> Added this logic to return adversarial samples in case of failed attack as well
            print(
                f"Attack failed/exceeded epsilon (dist={dist:.4f}). Clamping to eps={epsMax}"
            )
