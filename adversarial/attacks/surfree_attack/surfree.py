...
class SurFree:
    ...
    def __call__(self, model, X, labels, starting_points=None, **kwargs):
        ...
        # Get Starting Point
        self.best_advs = (
            attack.get_init_with_noise(model, X, labels)
        ...
        self.best_advs = torch.where(
            utils.atleast_kdim(self._images_finished, len(X.shape)), X, self.best_advs
        )
        ...
        self._directions_ortho = {
            i: v.unsqueeze(0) / v.norm() for i, v in enumerate(fd)
        }

        # Load Basis
        self._basis = (
            Basis(self.X, **kwargs["basis_params"])
        ...
        for step_i in range(self._steps):
            ...
            is_success = distance(best_candidates, self.X) < distance(
                self.best_advs, self.X
            )
            self.best_advs = torch.where(
                utils.atleast_kdim(
                    is_success * self._images_finished.logical_not(),
                    len(best_candidates.shape),
                ),
                best_candidates,
                self.best_advs,
            )
    ...
    def _get_candidates(self):
        ...
        while (epsilon == 0).any():
            epsilon = torch.where(
                self._images_finished, torch.ones_like(epsilon), epsilon
            )
            if not (epsilon == 0).any():
                break

            new_directions = self._basis.get_vector(
                self._directions_ortho,
                indexes=[i for i, eps in enumerate(epsilon) if eps == 0],
            )

            direction_2 = torch.where(
                utils.atleast_kdim(epsilon == 0, len(direction_2.shape)),
                new_directions,
                direction_2,
            )
                    ...
                    self._directions_ortho[i] = torch.cat(
                        (
                            ...
                            self._directions_ortho[i][
                                1 + len(self._directions_ortho[i]) - self.n_ortho :
                            ],
                            direction_2[i].unsqueeze(0),
                        ),
                        dim=0,
                    )
            ...
            self.theta_max = torch.where(
                new_epsilons == 0, self.theta_max * self.rho, self.theta_max
            )
            self.theta_max = torch.where(
                (new_epsilons != 0) * (epsilon == 0),
                self.theta_max / self.rho,
                self.theta_max,
            )
            epsilon = torch.where(
                (new_epsilons != 0) * (epsilon == 0), new_epsilons, epsilon
            )
        ...
        candidates = torch.cat(
            [function_evolution(eps).unsqueeze(0) for eps in epsilon], dim=0
        )

        if self.with_interpolation:
            d = (self.best_advs - self.X).flatten(1).norm(dim=1)
            delta = (
                (self._binary_search(candidates[1], boost=True) - self.X)
                .flatten(1)
                .norm(dim=1)
            )
    ...
    def _get_evolution_function(self, direction_2):
        ...
        if self.clip:
            return lambda theta: (
                self.X
                + self._add_step_in_circular_direction(
                    direction_1, direction_2, distances, theta
                )
            ).clip(0, 1)
        else:
            return lambda theta: (
    ...
    def _add_step_in_circular_direction(
        ...
        degree = utils.atleast_kdim(degree, len(direction1.shape))
        r = utils.atleast_kdim(r, len(direction1.shape))
        results = (
            torch.cos(degree * np.pi / 180) * direction1
            + torch.sin(degree * np.pi / 180) * direction2
        )
    ...
    def _get_best_theta(self, function_evolution, mask):
        ...
        for i, coeff in enumerate(coefficients):
            ...
            x = torch.where(
                utils.atleast_kdim((best_params == 0) * mask, len(self.X.shape)),
                x_evol,
            ...
            best_params = torch.where(
                (best_params == 0) * mask * is_advs, params, best_params
            )
    ...
    def _alpha_binary_search(self, function_evolution, lower):
        ...
        while mask_upper.any():
            ...
            x = torch.where(
                utils.atleast_kdim(mask_upper, len(self.X.shape)),
                new_upper_x,
                torch.zeros_like(self.X),
            )
            ...
            is_advs = self._is_adversarial(x)
            lower = torch.where(
                torch.logical_and(mask_upper, is_advs), new_upper, lower
            )
            upper = torch.where(
                torch.logical_and(mask_upper, is_advs.logical_not()), new_upper, upper
            )
        ...
        while step < self._BS_max_iteration and over_gamma.any():
            ...
            mid = torch.where(
                utils.atleast_kdim(
                    torch.logical_and(mid_bound != 0, over_gamma), len(self.X.shape)
                ),
            ...
            mid_opp = torch.where(
                utils.atleast_kdim(
                    torch.logical_and(check_opposite, over_gamma), len(mid.shape)
                ),
                function_evolution(-mid_bound),
            ...
            lower = torch.where(
                mask * over_gamma * is_adv.logical_not() * check_opposite * is_adv_opp,
                -mid_bound,
                lower,
            )
            upper = torch.where(
            ...
            upper = torch.where(
                mask * over_gamma * (abs(lower) != abs(mid_bound)), mid_bound, upper
            )

            check_opposite = (
                mask * over_gamma * check_opposite * is_adv_opp * (lower > 0)
            )
            over_gamma = abs(get_alpha(upper) - get_alpha(lower)) > self._BS_gamma
    ...
    def _binary_search(self, perturbed: torch.Tensor, boost=False) -> torch.Tensor:
        ...
        mask = utils.atleast_kdim(self._images_finished, len(perturbed.shape))

        # Boost Binary search
        if boost:
            boost_vec = torch.where(mask, perturbed, 0.2 * self.X + 0.8 * perturbed)
            is_advs = self._is_adversarial(boost_vec)
            is_advs = utils.atleast_kdim(is_advs, len(self.X.shape))
        ...
        iteration = 0
        while (
            torch.any(highs - lows > thresholds) and iteration < self._BS_max_iteration
        ):
            iteration += 1
            mids = (lows + highs) / 2
            epsilon = utils.atleast_kdim(mids, len(originals.shape))
        ...
        epsilon = utils.atleast_kdim(highs, len(originals.shape))
        return torch.where(
            mask, perturbed, (1.0 - epsilon) * originals + epsilon * perturbed
        )
...
class Basis:
    def __init__(
        ...
        **kwargs
        ...
        """
        self.X = originals
        self._f_dct2 = lambda a: dct.dct_2d(a)
        self._f_idct2 = lambda a: dct.idct_2d(a)
    ...
    def get_vector(self, ortho_with=None, indexes=None) -> torch.Tensor:
        ...
        vectors = [
            (
                self._gram_schmidt(r[i], ortho_with[i])
                if ortho_with[i] is not None
                else r[i]
            )
    ...
    def _load_params(
        ...
        if self.basis_type == "dct":
            ...
            dct_mask = self.get_zig_zag_mask(frequence_range, mask_size).to(
                self.X.device
            )
